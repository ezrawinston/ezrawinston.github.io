...
function handleGuessSubmit(event, secretWord) {
    ...
    checkWinInner(secretWord,guess)
    if (won) {
        for(let i = 0; i < secretWord.length; i++) {
            updateAnswer(secretWord[i], secretWord)
        }
    }  else {
        const errorOverlay = document.getElementById('wrong-overlay');
    ...
    // if won reveal letters, otherwise display incorrect
}


function enforceMaxLengthAndLetters(input) {
    const value = input.value;

    // Remove any non-letter characters
    input.value = value.replace(/[^a-zA-Z]/g, '');

    // Limit the length to 5 letters
...
function checkWinInner(secretWord, revealedLetters) {
...
function checkWin(secretWord) {
    ...
    checkWinInner(secretWord, revealedLetters)
}
